    ...
    def generate(self, template_path: str, source_image_path: str = None,
        ...
        if text_layers and texts:
            ...
            try:
                ...
                # 전체 그룹의 높이 = 첫 번째 텍스트 높이 + 간격 + 두 번째 텍스트 높이
                total_group_height = sum(text_heights) + text_gap * (len(text_heights) - 1)

                # 전체 이미지의 세로 중앙 (1350 / 2 = 675)
                image_center_y = canvas_size[1] // 2

                # 그룹의 시작 위치 (그룹을 전체 이미지 중앙에 배치)
                group_start_y = image_center_y - total_group_height // 2

                # 첫 번째 텍스트 위치 (매장명)
                if len(text_layers) > 0 and text_heights[0] > 0:
                    text_layers[0]['position'] = [
                        text_layers[0]['position'][0],
                        group_start_y + text_heights[0] // 2
                    ]

                # 두 번째 텍스트 위치 (주소)
                if len(text_layers) > 1 and text_heights[1] > 0:
                    text_layers[1]['position'] = [
                        text_layers[1]['position'][0],
                        group_start_y + text_heights[0] + text_gap + text_heights[1] // 2
                    ]

            except Exception as e:
                print(f"텍스트 그룹 중앙 정렬 계산 실패 (기본 위치 사용): {e}")
